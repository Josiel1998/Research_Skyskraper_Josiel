...
def main():
    ...
    data = pd.read_csv("GitHub/Research_Skyskraper_Josiel/SourceCode/Datasets/Investigation/AllSentences_FSB.csv") 
    for tweet in data.values:
        print(count)
        print(tweet[1])
        result = RateSentimentTri(tweet[1])
        resultN = RateSentiment(tweet[1])
        resN.append(resultN)
        res.append(result)
        count = count + 1
    ...
    print(data.head())
    print(data.info())
    data.to_csv("GitHub/Research_Skyskraper_Josiel/SourceCode/Datasets/Investigation/AllSentences_FSB.csv")
